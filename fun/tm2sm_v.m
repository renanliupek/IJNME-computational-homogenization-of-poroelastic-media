...
elseif order == 3   % TENSOR MATRIX WITH 3rd-ORDER COMPONENTS
       % case 2D
    if dim==2
...
elseif order == 4   % TENSOR MATRIX WITH 4th-ORDER COMPONENTS
    % case 2D
    if dim==2

    if size(t) ~= [1 1]; error('sorry, not supported for a matrix of 4th-order tensors'); end
